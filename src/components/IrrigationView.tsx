...
// Programmes d'irrigation automatiques (été, un jour sur deux) — m³ fixes par passage.
const PROGRAMS: { name: string; m3: number; detail: string }[] = [
  { name: 'Programme B', m3: 396, detail: 'Parcelles A + B1 + B2' },
  { name: 'Programme C', m3: 289, detail: 'Nouveaux vergers D1/D2/E (102) + Parcelle C (147) + Jardin (40)' },
]
const PROGRAM_M3: Record<string, number> = Object.fromEntries(PROGRAMS.map(p => [p.name, p.m3]))
const PROGRAM_NAMES = PROGRAMS.map(p => p.name)
const ROWS = [...PROGRAM_NAMES, ...PARCELS]
const ROW_COLORS: Record<string, string> = { 'Programme B': '#0a5c47', 'Programme C': '#a84a17', ...PARCEL_COLORS }
...
export default function IrrigationView({ supabase, userId }: { supabase: SupabaseClient; userId: string }) {
  ...
  const [form, setForm] = useState({ date: today(), parcel: 'Programme B', hours: '', flow: '', m3: String(PROGRAM_M3['Programme B']) })
  ...
  const onParcel = (p: string) => setForm(f => {
    // Programme : m³ fixe par passage (pas de débit/temps)
    if (PROGRAM_M3[p] != null) return { ...f, parcel: p, flow: '', m3: String(PROGRAM_M3[p]) }
  ...
  // Pivot ligne (programme/parcelle) × mois (m³) pour l'année sélectionnée
  const pivot = useMemo(() => {
    const grid: Record<string, number[]> = {}
    ROWS.forEach(p => grid[p] = Array(12).fill(0))
    logs.forEach(l => {
  ...
  // N'affiche que les lignes réellement utilisées (programmes + parcelles ayant des relevés)
  const activeRows = ROWS.filter(p => pivot[p].some(v => v > 0))
  const displayRows = activeRows.length ? activeRows : PROGRAM_NAMES
  const monthTotals = MONTHS.map((_, m) => displayRows.reduce((s, p) => s + pivot[p][m], 0))
  const rowTotals = Object.fromEntries(ROWS.map(p => [p, pivot[p].reduce((s, v) => s + v, 0)]))
  ...
  const isProgram = PROGRAM_M3[form.parcel] != null
        ...
        <>
          {flowMissing && (
            <div style={{ ...card, background: '#fff8ec', color: '#9a6b15', fontSize: 13 }}>
              ⚙️ Les <strong>programmes</strong> B et C ont un volume fixe pré-rempli. Pour une saisie <strong>par parcelle</strong>, les débits (m³/h) ne sont pas encore renseignés : tape le débit à la main, ou envoie-les moi pour un calcul automatique.
            </div>
          )}
          ...
          <form onSubmit={submit} style={card}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 12, alignItems: 'end' }}>
              ...
              <div><label style={label}>Programme / Parcelle</label>
                <select style={input} value={form.parcel} onChange={e => onParcel(e.target.value)}>
                  <optgroup label="Programmes (été, 1 jour sur 2)">
                    {PROGRAMS.map(p => <option key={p.name} value={p.name}>{p.name} — {p.m3} m³</option>)}
                  </optgroup>
              ...
              <div><label style={label}>Temps (h)</label><input style={input} inputMode="decimal" value={form.hours} onChange={e => onHours(e.target.value)} placeholder="ex : 3" disabled={isProgram} /></div>
              <div><label style={label}>Débit (m³/h)</label><input style={input} inputMode="decimal" value={form.flow} onChange={e => onFlow(e.target.value)} placeholder="ex : 12" disabled={isProgram} /></div>
            ...
            {isProgram && (
              <div style={{ marginTop: 10, fontSize: 12, color: 'var(--muted)' }}>
                📋 {PROGRAMS.find(p => p.name === form.parcel)?.detail} · volume fixe {PROGRAM_M3[form.parcel]} m³ par passage (été, 1 jour sur 2).
              </div>
            )}
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 720 }}>
                ...
                <tbody>
                  {displayRows.map(p => (
                    <tr key={p}>
                      <td style={{ padding: '6px 8px', fontSize: 13, fontWeight: 600, color: ROW_COLORS[p] || '#555' }}>{p}</td>
            ...
            <div style={{ width: '100%', height: 300 }}>
              <ResponsiveContainer>
                <BarChart data={chartData} margin={{ top: 6, right: 10, left: 0, bottom: 0 }}>
                  ...
                  {displayRows.map(p => <Bar key={p} dataKey={p} stackId="a" fill={ROW_COLORS[p] || '#999'} />)}
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
              ...
              <ResponsiveContainer>
                <BarChart data={annualData} margin={{ top: 6, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <Bar dataKey="m3" radius={[4, 4, 0, 0]}>
                    {annualData.map(d => <Cell key={d.parcel} fill={ROW_COLORS[d.parcel] || '#999'} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
